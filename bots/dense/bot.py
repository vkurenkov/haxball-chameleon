...
from bots.dense.preprocessing import game_state_to_numpy, Encoder
...
class DenseBot(interactive.Interactive):
	def __init__(self, channel_id):
		...
		self.model = load_model("imitator2.h5")
		self.recent_frames = []
	...
	def act(self, obs: np.array) -> int:
		action = np.argmax(self.model.predict(obs.reshape(1, -1)))
		return action