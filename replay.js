...
function closure() {
  ...
  function $_ReplayController(a, b, c) {
    /** @type {$_Writer} */
    this.$__smallStateWriter = $_Writer.ca();
...
function _test() {
  const sub = (x, y) => `${x}: ${y}`;
  function check_game_state(report, gameState, expected) {
    const writer = exposed.$_Writer.ca();
    gameState.aa(writer);
    const actual = new Uint8Array(writer.mg());

    if (actual.byteLength !== expected.byteLength) {
      report('length differ');
      return false;
    }
    if (Array.from(actual).some((a, i) => a !== expected[i])) {
      report('bytes differ');
      return false;
    }
    return true;
  }
  function test_replay(report, replay, begin, end) {
    const controller = new exposed.$_ReplayController(replay, new exposed.$_GameState, 2);
    if (!check_game_state(sub(report, 'begin'), controller.L, begin)) {
      return false;
    }
    controller.td = controller.Te * controller.Mg;
    while (controller.td > 0) {
      controller.v();
    }
    return check_game_state(sub(report, 'end'), controller.L, end);
  }
  function test_state_repack(report, original) {
    const gameState = new exposed.$_GameState();
    gameState.ea(new exposed.$_Reader(new DataView(original.buffer)));
    return check_game_state(report, gameState, original);
  }
  const load = (rel) => new Uint8Array(require('fs').readFileSync(path.join('../dataset', rel)));

  const dataset = {
    replay1: load('replay1.hbr2'),
    replay1_begin: load('replay1-begin.bin'),
    replay1_end: load('replay1-end.bin'),
    _1549310281654: load('1549310281654.hbr2'),
    _1549310281654_begin: load('1549310281654-begin.bin'),
    _1549310281654_end: load('1549310281654-end.bin'),
  };

  if (![
    test_state_repack('replay1-begin', dataset.replay1_begin),
    test_state_repack('replay1-end', dataset.replay1_end),

    test_state_repack('1549310281654-begin', dataset._1549310281654_begin),
    test_state_repack('1549310281654-end', dataset._1549310281654_end),

    test_replay('replay1', dataset.replay1, dataset.replay1_begin, dataset.replay1_end),
    test_replay('1549310281654', dataset._1549310281654, dataset._1549310281654_begin, dataset._1549310281654_end),
  ].every(x => x)) {
    return;
  }
}
...
function watch() {
  function reload() {
    ...
    require(module.filename).test();
  }
  let timeout = null;
  fs.watch(module.filename, (e) => {
    if (e === 'change') {
      if (timeout !== null) {
...
function convert(replay) {
  ...
  controller.onTick = () => {
    ...
    writer.$_f64(controller.S * controller.vc); // replay time
    if (!L.H) {
      writer.$_u8(0); // 0: menu
    } else {
      writer.$_u8(L.H.Ga > 0 ? 1 : L.H.zb == 0 ? 2 : L.H.zb == 1 ? 3 : 4); // 1: pause, 2: warmup, 3: game, 4: goal
      writer.$_f64(L.H.Ac); // game time
      writer.$_u8(L.H.Kb); // red score
      writer.$_u8(L.H.Cb); // blue score
      writer.$_bool(L.H.xa > 0 && L.H.Ac > L.H.xa); // overtime
      const players = L.D.filter(x => x.$.P !== 0);
      writer.$_u8(players.length); // player count
      for (const player of players) {
        writer.$_u8(player.T); // id
        writer.$_u8(player.mb); // input
        writer.$_bool(player.bc); // kick
        writer.$_u8(player.$.P - 1); // team, 0: red, 1: blue
        writer.$_disc(player.F); // player disc
      }
      writer.$_disc(L.H.wa.K[0]); // ball disc
    }
  };
  controller.onTick();
  controller.td = controller.Te * controller.Mg;
...
if (require.main === module) {
  switch (process.argv[2]) {
    ...
    default: {
      ...
      console.log(['watch', 'test', 'convert INPUT OUTPUT', 'help', '--help', '-h'].map(x => `node ${module.id} ${x}`).join('\n'));
    }
  }
}
